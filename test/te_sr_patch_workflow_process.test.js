...
describe('TE_SR PATCH workflow trigger', () => {
  ...
  it('triggers workflow when submitting without REQUEST_ID', async () => {
    ...
    const req = {
      data: { REQ_TXN_ID: id, DECISION: 'submit', REQUEST_ID: '' },
      user: { id: 'tester' },
      warn: () => {},
    }
    ...
    const record = await SELECT.one.from(MON_WF_PROCESS)
    assert.strictEqual(record.REQ_TXN_ID, id)
    assert.strictEqual(record.REQUEST_ID, req.data.REQUEST_ID)
  })
})
